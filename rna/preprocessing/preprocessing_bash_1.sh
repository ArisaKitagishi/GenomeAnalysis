...
do
	...
	if [ ! -d $file_path ]; then
		...
		trimmomatic PE -phred33 $rna_rawdata_forward $rna_rawdata_reverse "${file}_paired_1.fastq.gz" "${file}_unpaired_1.fastq.gz" "${file}_paired_2.fastq.gz" "${file}_unpaired_2.fastq.gz" ILLUMINACLIP:$TRIMMOMATIC_ROOT/adapters/TruSeq3-PE.fa:2:30:10:2:True
